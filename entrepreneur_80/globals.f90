...
contains
  ...
  function valuefunc_w(xy)
    ...
    inctax_com = tarif(max(wage*l_com - 0.10d0*wage*l_com - 0.04d0*inc_bar(0) - pencon_com, 0d0) + pen(ip_com, ij_com, it_com))

    ! calculate capital gains tax
    captax_com = taur(it_com)*1.055d0*max(r(it_com)*a(ia_com)-0.10d0*r(it_com)*a(ia_com)-2d0*0.0267d0*inc_bar(0), 0d0)
  ...
  function valuefunc_e(xy)
    ...
    ! calculate income tax
    inctax_com = tarif(max(profit - 0.10d0*profit - 0.04d0*inc_bar(0) - pencon_com, 0d0) + pen(ip_com, ij_com, it_com))

    ! calcualte capital gains tax
    captax_com = taur(it_com)*1.055d0*max(r(it_com)*max(a(ia_com)-k_com, 0d0) - 0.10d0*r(it_com)*max(a(ia_com)-k_com, 0d0) - 2d0*0.0267d0*inc_bar(0), 0d0)

    ! calculate consumption
    c_com =  (a(ia_com) + r(it_com)*max(a(ia_com)-k_com, 0d0) + profit + beq(is_com, ij_com, it_com) + pen(ip_com, ij_com, it_com) + p_hat + v_ind  &
           - captax_com - inctax_com - pencon_com - mx_com - a_plus)*(1d0-swc)*pinv(it_com)
    ...
    valuefunc_help = 0d0
    oplus_com = 0d0
    cx_com = swc*c_com/(1d0-swc)/pinv(it_com)
