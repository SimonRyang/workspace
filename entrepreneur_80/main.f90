...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.95666d0, 0.0d0, 0.02321d0, eta(:, 1), pi_eta(:, :, 1), dist_eta(:, 1))
    eta(:, 1) = exp(eta(:, 1))/sum(dist_eta(:, 1)*exp(eta(:, 1)))

    call discretize_AR(0.95687d0, 0.0d0, 0.02812d0, eta(:, 2), pi_eta(:, :, 2), dist_eta(:, 2))
    eta(:, 2) = exp(eta(:, 2))/sum(dist_eta(:, 2)*exp(eta(:, 2)))

    call discretize_AR(0.95828d0, 0.0d0, 0.03538d0, eta(:, 3), pi_eta(:, :, 3), dist_eta(:, 3))
    eta(:, 3) = exp(eta(:, 3))/sum(dist_eta(:, 3)*exp(eta(:, 3)))

    ! initialize entrepreneurial ability
    call discretize_AR(0.9400d0, -0.600d0, 0.0370d0, theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.9400d0, -0.550d0, 0.0380d0, theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
    theta(:, 2) = exp(theta(:, 2))!/sum(dist_theta(:, 2)*exp(theta(:, 2)))

    call discretize_AR(0.9410d0, -0.400d0, 0.0395d0, theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
  ...
  subroutine get_distribution(it)
    ...
    do is = 1, NS
      do ie = 1, NE
        do iw = 1, NW
          m(0, 0, 0, 0, 4, 4, is, 1, it) = dist_skill(is) !dist_eta(iw, is)*dist_theta(ie, is)*dist_skill(is)
        enddo ! iw
      enddo ! ie
    enddo ! is
                ...
                do ia = 0, NA
                  do io = 0, NO
                    ...
                    CC(it) = CC(it) + c(io, ia, ix, ip, iw, ie, is, ij, it) &
                              *m(io, ia, ix, ip, iw, ie, is, ij, it)
                    SC(it) = SC(it) + cx(io, ia, ix, ip, iw, ie, is, ij, it) &
                              *m(io, ia, ix, ip, iw, ie, is, ij, it)
  ...
  subroutine output(it)
    ...
    do is = 1, NS
      punb(is, 1) = psi(is, 1)
      life_exp(is) = life_exp(is) + 21d0*punb(is, 1)*(1d0-psi(is, 2))
      do ij = 2, JJ
        punb(is, ij) = punb(is, ij-1)*psi(is, ij)
        life_exp(is) = life_exp(is) + (21d0 + dble(ij-1))*punb(is, ij)*(1d0-psi(is, ij+1))
      enddo ! ij
    enddo ! is
