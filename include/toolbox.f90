...
real*8,  private  :: tbox_gftol = 1d-10

! Maximum number of iterations
integer, private  :: tbox_itermax_min = 200

! Maximum number of iterations for brent_pow
integer, parameter, private  :: tbox_tbox_itermax_pow_b = 250
...
!##############################################################################
    ...
    subroutine log_normal_discrete_1(x, prob, mu, sigma)
        ...
        if(sigma_c <= 0d0)then
            call error('log_normal_discrete','sigma has zero or negative value')
        endif
        if(mu_c <= 0d0)then
...
!##############################################################################
    ...
    ! Plots a histogram.
    !##############################################################################
    subroutine plot_hist_new(xvalues, nbins, left, right, absolute, color, legend)
