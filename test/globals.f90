...
module globals
    ...
    real*8, parameter :: beta  = (1d0-0.002d0)**5
    ...
    real*8, parameter :: delta = 1d0-(1d0-0.0825d0)**5
    ...
    real*8 :: c_coh(JJ), h_coh(JJ), l_coh(JJ), a_coh(JJ)
    real*8 :: v_coh(JJ)
...
contains
    ...
    function foc(x)
        ...
        real*8 :: foc(size(x, 1)), a_plus, varphi, tomorrow, wage, available
        integer :: ial, iar
        ...
        tomorrow = max(varphi*RHS(ij_com+1, ial, ip_com, is_com) + &
                            (1d0-varphi)*RHS(ij_com+1, iar, ip_com, is_com), 0d0)

        ! calculate first order condition for consumption       
        foc(1) = margu(cons_com, lab_com)**(-gamma) - tomorrow + x(2)
