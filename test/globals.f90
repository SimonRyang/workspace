...
module globals
    ...
    real*8 :: gamma, nu, beta

    ! production parameters
    real*8 :: alpha, delta, Omega

    ! numerical parameters
    real*8 :: a_l, a_u, a_grow, damp, sig
    integer :: itermax
    ...
    real*8 :: tauc(0:TT), tauw(0:TT), taur(0:TT), taup(0:TT), all(0:TT), kappa(0:TT)
    ...
    real*8 :: c_coh(JJ, 0:TT), h_coh(JJ, 0:TT), l_coh(JJ, 0:TT), a_coh(JJ, 0:TT)
    real*8 :: v_coh(JJ, 0:TT), vv_coh(JJ, 0:TT) = 0d0

    ! the shock process
    real*8 :: sigma_theta, sigma_eps, rho
    ...
    integer :: JR
    real*8 :: n_p, rpop(JJ, 0:TT), pop(JJ, 0:TT), eff(JJ), workpop(0:TT)
...
contains
    ...
    function foc(x)
        ...
        real*8 :: foc(size(x, 1)), a_plus, varphi, tomorrow, wage, v_ind, available
        integer :: ial, iar, itp
        ...
        if(ij_com < JR)available = available + all(it_com)

        ! determine labor
        if(ij_com < JR)then
            lab_com = min( max( (1d0-nu)*(a_plus-available)/wage + nu, 0d0) , 1d0-1d-10)
        else
            lab_com = 0d0
        endif
        ...
        foc(1) = margu(cons_com, lab_com, it_com)**(-gamma) - tomorrow + x(2)

        ! get complementary slackness condition
        foc(2) = x(1) + x(2) - sqrt(x(1)**2 + x(2)**2)
