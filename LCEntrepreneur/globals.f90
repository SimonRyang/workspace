module globals
    ...
    real*8, parameter :: k_u = 0.75d0*X_u/(1d0-xi)
  ...
  subroutine solve_consumption(ij, ia, ip, ik, iw, ie, io_p)
      ...
      if (io_p == 0) then
        call fminsearch(x_in, fret, (/X_l, 0d0/), (/X_u, 0.99d0/), cons_w)
      else
        call fminsearch(x_in, fret, (/X_l, 0d0/), (/X_u, 0.99d0/), cons_e)
      endif
  ...
  function inv_o(x_in)
    ...
    function cons_w(x_in)
        ...
        if(cons_com <= 0d0)then
           cons_w = -1d-16**egam/egam*(1d0+abs(cons_com))
        elseif(lab_com < 0d0) then
          cons_w = -1d-16**egam/egam*(1d0+abs(lab_com))
        elseif(lab_com >= 1d0) then
          cons_w = -1d-16**egam/egam*lab_com
        else
    ...
    function cons_e(x_in)
        ...
        if(cons_com <= 0d0)then
           cons_e = -1d-16**egam/egam*(1d0+abs(cons_com))
        elseif(lab_com < 0d0) then
          cons_e = -1d-16**egam/egam*(1d0+abs(lab_com))
        elseif(lab_com >= 1d0) then
          cons_e = -1d-16**egam/egam*lab_com
        else
