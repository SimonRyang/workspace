  ...
  function cons_e(x_in)
      ...
      if (ik_com == 0) then
        ...
        if (income /= w*eff(ij_com)*eta(iw_com)*lab_com) then
          write(*,*)'Error1!', io, income, w*eff(ij_com)*eta(iw_com)*lab_com, lab_com
          call sleep(1)
      endif
      ...
      ! maximize value function for current owner (next period owner)
        ...
        if (income /= theta(ie_com)*(k(ik_com)**alpha*(eff(ij_com)*lab_com)**(1d0-alpha))**nu + (1d0-delta_k)*k(ik_com)) then
          write(*,*)'Error2!', io, income , theta(ie_com)*(k(ik_com)**alpha*(eff(ij_com)*lab_com)**(1d0-alpha))**nu + (1d0-delta_k)*k(ik_com), lab_com
          call sleep(1)
        endif
