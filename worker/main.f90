  ...
  subroutine output_summary()
        ...
        do ip = 0, NP
          do is = 1, NS
            do iw = 1, NW
              ...
              HEV_help = ((VV(ia, ix, ip, is, iw, ij, 1)/max(VV(ij, ia, ix, ip, is, iw, 0), -1d10))**(1d0/(1d0-1d0/gamma))-1d0)*100d0
    ...
    do it = 1, TT
      write(22,'(i3,22f8.2,f10.5)')it,(/AA(it)/AA(0)-1d0, KK(it)/KK(0)-1d0, KC(it)/KC(0)-1d0, LC(it)/LC(0)-1d0, &
        ...
        II(it)/II(0)-1d0, YY(it)/YY(0)-1d0, YC(it)/YC(0)-1d0, tauc(it)-tauc(0), taup(it)-taup(0), PP(it)/YY(it)-PP(0)/YY(0)/)*100d0, &
