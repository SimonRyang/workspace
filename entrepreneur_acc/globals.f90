module globals
  ...
  ! LSRA variables
  real*8 :: BA = 0d0, SV = 0d0, lsra_comp, lsra_all, Vstar
  logical :: lsra_on
  ...
  real*8 :: theta(NS, NE), dist_theta(NS, NE), pi_theta(NS, NE, NE)
  ...
  ! numerical variables
  integer :: io_com, is_com, ie_com, iw_com, ip_com, ia_com, ij_com
  ...
  !$omp threadprivate(io_com, is_com, ie_com, iw_com, ip_com, ia_com, ij_com)
...
contains
  ...
  function valuefunc_w(xy)
    ...
    if (labor) then
      ...
      l_com = l(io_com, ij_com, ia_com, ip_com, is_com, iw_com, ie_com)
    endif

    ! calculate the wage rate and next periods pension claims
    wage = w*eff(ij_com, is_com)*eta(is_com, iw_com)
    ...
    if (ij_com < JJ) then
      ...
      valuefunc_w = (varphi*varpsi*EV(0, is_com, ie_com, iw_com, ipl, ial, ij_com+1) &
               + varphi*(1d0-varpsi)*EV(0, is_com, ie_com, iw_com, ipr, ial, ij_com+1) &
               + (1d0-varphi)*varpsi*EV(0, is_com, ie_com, iw_com, ipl, iar, ij_com+1) &
               + (1d0-varphi)*(1d0-varpsi)*EV(0, is_com, ie_com, iw_com, ipr, iar, ij_com+1)) &
              **(1d0-gamma)/(1d0-gamma)

      ! interpolate next period's value function as an entrepreneur
      if (ij_com < JR-1) then

        valuefunc_help = (varphi*varpsi*EV(1, is_com, ie_com, iw_com, ipl, ial, ij_com+1) &
                  + varphi*(1d0-varpsi)*EV(1, is_com, ie_com, iw_com, ipr, ial, ij_com+1) &
                  + (1d0-varphi)*varpsi*EV(1, is_com, ie_com, iw_com, ipl, iar, ij_com+1) &
                  + (1d0-varphi)*(1d0-varpsi)*EV(1, is_com, ie_com, iw_com, ipr, iar, ij_com+1)) &
                 **(1d0-gamma)/(1d0-gamma)
  ...
  function valuefunc_e(xy)
    ...
    if (ij_com < JJ) then
      ...
      valuefunc_e = (varphi*varpsi*EV(0, is_com, ie_com, iw_com, ipl, ial, ij_com+1) &
               + varphi*(1d0-varpsi)*EV(0, is_com, ie_com, iw_com, ipr, ial, ij_com+1) &
               + (1d0-varphi)*varpsi*EV(0, is_com, ie_com, iw_com, ipl, iar, ij_com+1) &
               + (1d0-varphi)*(1d0-varpsi)*EV(0, is_com, ie_com, iw_com, ipr, iar, ij_com+1)) &
              **(1d0-gamma)/(1d0-gamma)
  ...
  function valuefunc_r(xy)
    ...
    if (ij_com < JJ) then
      ...
      valuefunc_r = (varphi*EV(0, is_com, ie_com, iw_com, ip_com, ial, ij_com+1) &
               + (1d0-varphi)*EV(0, is_com, ie_com, iw_com, ip_com, iar, ij_com+1 )) &
               **(1d0-gamma)/(1d0-gamma)
      ...
      do ia = 0, NA
        do ip = 0, NP
          do iw = 1, NW
            do ie = 1, NE
              do is = 1, NS
                if (m(0, is, ie, iw, ip, ia, ij) > 0d0) iamax(ij)=ia
                if (m(1, is, ie, iw, ip, ia, ij) > 0d0) iamax(ij)=ia
              enddo ! is
            enddo ! ie
          enddo ! iw
        enddo ! ip
            ...
            do ie = 1, NE
              do is = 1, NS
                ...
                if(m(0, is, ie, iw, ip, ia, ij) > 0d0) then
                  ...
                  xs(ic) = a(ia)*r + pen(ip, ij) + eff(ij, is)*eta(is, iw)*l(0, is, ie, iw, ip, ia, ij)*w
                  ic = ic + 1
                endif
                if(m(1, is, ie, iw, ip, ia, ij) > 0d0) then
                  ys(ic) = m(1, is, ie, iw, ip, ia, ij)
                  xs(ic) = max(a(ia)-k(1, is, ie, iw, ip, ia, ij), 0d0)*r + pen(ip, ij) &
                           + profent(k(1, is, ie, iw, ip, ia, ij), ij, ia, is, ie)
                  ic = ic + 1
                endif
