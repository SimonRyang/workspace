module globals
    ...
    real*8, parameter :: Q_grow = 0.0025d0
  ...
  subroutine solve_consumption(io_p, ia, ik, ix, ip, iw, ie, ij)
      ...
      c_t(io_p, ia, ik, ix, ip, iw, ie, ij) =  cons_com !aas_com - x_in(1)
  ...
  function inv_e(x_in)
    ...
    function cons_o(x_in)
        ...
        p_plus_com = (p(ip_com)*dble(ij_com-1) + mu*(lambda+(1d0-lambda)*min(income/ybar, 2d0)))/dble(ij_com)

        if (p_plus_com > 2d0) write(*,*) 'p_plus_com:', p_plus_com
