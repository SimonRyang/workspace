...
contains
  ...
  subroutine solve_retiree(iq_p, ik, ix, ip_p, iw, ie, is, ij, it)
    ...
    EV_temp = varphi_a*varphi_x            *EV(ial, 0, ixl, ip_p, iw, ie, is, ij+1, itp) + &
              varphi_a*(1d0-varphi_x)      *EV(ial, 0, ixr, ip_p, iw, ie, is, ij+1, itp) + &
              (1d0-varphi_a)*varphi_x      *EV(iar, 0, ixl, ip_p, iw, ie, is, ij+1, itp) + &
              (1d0-varphi_a)*(1d0-varphi_x)*EV(iar, 0, ixr, ip_p, iw, ie, is, ij+1, itp)
  ...
  subroutine solve_consumption(io_p, ia, ik, ix, ip, iw, ie, is, ij, it)
    ...
    if (ij < JR) then
      ...
      !call fminsearch(x_in(1), fret, Q_l, Q_u, cons_o)
    else
    ...
    if((1d0+r(it))*a(ia)+eff(is,ij)*w(it)*eta(iw, is)*lab_com-x_in(1)-cons_com-tauc(it)*cons_com > 0d0) then
      write(*,*)'help',(1d0+r(it))*a(ia)+eff(is,ij)*w(it)*eta(iw, is)*lab_com-x_in(1)-cons_com-tauc(it)*cons_com
    endif
