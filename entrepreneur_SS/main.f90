...
program main
  ...
  mu_val(1, :) =  (/0.100d0, 0.150d0, 0.200d0, 0.250d0, 0.300d0/)
  mu_val(2, :) =  (/0.100d0, 0.150d0, 0.200d0, 0.250d0, 0.300d0/)
  mu_val(3, :) =  (/0.000d0, 0.050d0, 0.100d0, 0.150d0, 0.200d0/)
  sig_val(1, :) = (/0.0350d0, 0.0375d0, 0.0400d0, 0.0425d0, 0.0450d0/)
  sig_val(2, :) = (/0.0350d0, 0.0375d0, 0.0400d0, 0.0425d0, 0.0450d0/)
  sig_val(3, :) = (/0.0350d0, 0.0400d0, 0.0450d0, 0.0500d0, 0.0550d0/)
  rho_val(1, :) = (/0.940d0, 0.930d0, 0.950d0, 0.920d0, 0.910d0/)
  rho_val(2, :) = (/0.940d0, 0.930d0, 0.950d0, 0.920d0, 0.910d0/)
  rho_val(3, :) = (/0.945d0, 0.940d0, 0.942d0, 0.937d0, 0.945d0/)
...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(rho_val(1, rh1)**5d0, -mu_val(1, m1), sigma5(rho_val(1, rh1), sig_val(1, s1)), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))

    call discretize_AR(rho_val(2, rh2)**5d0, -mu_val(2, m2), sigma5(rho_val(2, rh2), sig_val(2, s2)), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
    theta(:, 2) = exp(theta(:, 2))

    call discretize_AR(rho_val(3, rh3)**5d0, -mu_val(3, m3), sigma5(rho_val(3, rh3), sig_val(3, s3)), theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
    theta(:, 3) = exp(theta(:, 3))
            ...
            do iw = 1, NW
              do ip = 0, NP
                do ia = 0, NA
                  ...
                  ! copy decisions
                  aplus(:, ia, ip, iw, ie, is, ij) = xy(1)
                  k(:, ia, ip, iw, ie, is, ij) = k_com
                  pplus(:, ia, ip, iw, ie, is, ij) = pplus_com
                  c(:, ia, ip, iw, ie, is, ij) = max(c_com, 1d-10)
                  cx(:, ia, ip, iw, ie, is, ij) = cx_com
                  l(:, ia, ip, iw, ie, is, ij) = l_com
                  oplus(:, ia, ip, iw, ie, is, ij) = oplus_com
                  pencon(:, ia, ip, iw, ie, is, ij) = pencon_com
                  inctax(:, ia, ip, iw, ie, is, ij) = inctax_com
                  captax(:, ia, ip, iw, ie, is, ij) = captax_com
                  VV(:, ia, ip, iw, ie, is, ij) = -fret
