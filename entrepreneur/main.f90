...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.9390d0**5d0, -0.400d0, sigma5(0.9390d0, 0.0365d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.9360d0**5d0, -0.350d0, sigma5(0.9360d0, 0.0370d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
