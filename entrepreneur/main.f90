...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.9400d0**5d0, -0.3205d0, sigma5(0.9400d0, 0.0370d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    ...
    call discretize_AR(0.9410d0**5d0, 0.0050d0, sigma5(0.9410d0, 0.0395d0), theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
    theta(:, 3) = exp(theta(:, 3))!/sum(dist_theta(:, 3)*exp(theta(:, 3)))

    ! initial guesses for macro variables
    taup(0) = 0.192d0 !0.192393d0
    tauc(0) = 0.173d0 !0.173856d0
    inc_bar(0) = 0.53d0 !0.527950d0
    bqs(:, 0) = 0.2d0 !(/0.019551d0, 0.119450d0, 0.302507d0/)
    BQ(0) = 0.44d0 !0.441508d0
    BB(0) = 0.95d0 !0.953949d0
    KC(0) = 3.8d0 !3.821249d0
    LC(0) = 5.2d0 !5.171536d0
...
!                    if (dVV_da < 0d0)write(*,*)'dVV_da', dVV_da
  ...
  subroutine output(it)
    ...
    write(23,'(a,4f8.2)')'<10', (/sum(m(1, :, :, :, :, :, 1, :, it))/sum(m(:, :, :, :, :, :, 1, :, it)), &
