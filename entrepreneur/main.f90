program main
  ...
  nu = 0.88d0
  l_bar = 0.47d0
  suc = 0.05d0 !(1d0 - 0.50d0)**(1d0-gamma)
  swc = 0.05d0 !(1d0 - 0.50d0)**(1d0-gamma)
  ! convert variables into per period values
...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.9400d0**5d0, -0.5000d0, sigma5(0.9400d0, 0.0370d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.9400d0**5d0, -0.450d0, sigma5(0.9400d0, 0.0380d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
