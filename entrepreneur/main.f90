program main
  ...
  swc = 0.75d0 !(1d0 - 0.50d0)**(1d0-gamma)
  ! convert variables into per period values
...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.9400d0**5d0, 0.370d0, sigma5(0.9400d0, 0.0380d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
    theta(:, 2) = exp(theta(:, 2))!/sum(dist_theta(:, 2)*exp(theta(:, 2)))

    call discretize_AR(0.9410d0**5d0, 0.560d0, sigma5(0.9410d0, 0.0395d0), theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
