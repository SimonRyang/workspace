...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.939d0**5d0, -0.600d0, sigma5(0.939d0, 0.0365d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.936d0**5d0, -0.550d0, sigma5(0.936d0, 0.0370d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
    theta(:, 2) = exp(theta(:, 2))!/sum(dist_theta(:, 2)*exp(theta(:, 2)))

    call discretize_AR(0.945d0**5d0, -0.300d0, sigma5(0.945d0, 0.0390d0), theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
  ...
  subroutine output(it)
    ...
    if (show_graphics) then
      ...
      if (it == 0 .OR. it == TT) then
        ...
        call plot((/(dble(ia), ia=0,NA)/), EV(0, :, 0, 0, 3, 3, 2, 5, it), marker=1)
        call plot((/(dble(ia), ia=0,NA)/), EV(1, :, 0, 0, 3, 3, 2, 5, it), marker=1)
        call plot((/(dble(ia), ia=0,NA)/), oplus(0, :, 0, 0, 3, 3, 2, 5, it), marker=2)
        call plot((/(dble(ia), ia=0,NA)/), oplus(1, :, 0, 0, 3, 3, 2, 5, it), marker=2)
        call execplot()

        call plot((/(dble(ia), ia=0,NA)/), EV(0, :, 0, 0, 3, 4, 2, 3, it), marker=1)
        call plot((/(dble(ia), ia=0,NA)/), EV(1, :, 0, 0, 3, 4, 2, 3, it), marker=1)
        call plot((/(dble(ia), ia=0,NA)/), oplus(0, :, 0, 0, 3, 4, 2, 3, it), marker=2)
        call plot((/(dble(ia), ia=0,NA)/), oplus(1, :, 0, 0, 3, 4, 2, 3, it), marker=2)
        call execplot()

        call plot((/(dble(ia), ia=0,NA)/), EV(0, :, 0, 0, 2, 5, 3, 5, it), marker=1)
        call plot((/(dble(ia), ia=0,NA)/), EV(1, :, 0, 0, 2, 5, 3, 5, it), marker=1)
