...
contains
  ...
  subroutine initialize()
        ...
        do ie = 1, NE
          do iw = 1, NW
            do ip = 0, NP
              do ix = 0, NX
                do ia = 0, NA
                  aplus(0, ia, ix, ip, iw, ie, is, ij, 0) = max(a(ia)/2d0, a(1, is))
                  aplus(1, ia, ix, ip, iw, ie, is, ij, 0) = max(a(ia)/2d0, a(1, is))
                enddo ! ia
              enddo ! ix
            enddo ! ip
          enddo ! iw
    ...
    call discretize_AR(0.939d0**5d0, -0.600d0, sigma5(0.939d0, 0.0370d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.936d0**5d0, -0.550d0, sigma5(0.936d0, 0.0380d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
