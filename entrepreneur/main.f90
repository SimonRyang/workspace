...
contains
  ...
  subroutine solve_household(ij_in, it_in)
    ...
    do ij = JJ, 1, -1
      ...
      if (ij >= JR) then
        ...
        !$omp parallel do copyin(ij_com, it_com, iw_com, ie_com, io_com) collapse(2) schedule(dynamic, 1) private(xy, fret)
        do ia = 0, NA
          do ix = 0, NX
            do ip = 0, NP
              do is = 1, NS
      ...
      elseif (ij >= 2) then
        ...
        !$omp parallel copyin(ij_com, it_com) private(xy, fret, limit)
      ...
      elseif (ij == 1) then
        ...
        io_com = 0
	      ...
	      !$omp parallel do copyin(ij_com, it_com, ia_com, ix_com, ip_com, io_com) collapse(3) schedule(dynamic, 1) private(xy, fret)
        do iw = 1, NW
          do ie = 1, NE
            do is = 1, NS
  ...
  subroutine interpolate(ij, it)
    ...
    !$omp parallel do collapse(2) schedule(dynamic,1) private(iw_p, ie_p)
    do ia = 0, NA
      do ix = 0, NX
        do ip = 0, NP
          do iw = 1, NW
  ...
  subroutine get_distribution(it)
    ...
    real*8 :: varpsi, varchi, varphi, tmp
