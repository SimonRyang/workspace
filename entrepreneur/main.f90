...
contains
  ...
  subroutine initialize()
    ...
    call discretize_AR(0.910d0**5d0, -0.100d0, sigma5(0.910d0, 0.0350d0), theta(:, 1), pi_theta(:, :, 1), dist_theta(:, 1))
    theta(:, 1) = exp(theta(:, 1))!/sum(dist_theta(:, 1)*exp(theta(:, 1)))

    call discretize_AR(0.920d0**5d0, -0.100d0, sigma5(0.920d0, 0.0375d0), theta(:, 2), pi_theta(:, :, 2), dist_theta(:, 2))
    theta(:, 2) = exp(theta(:, 2))!/sum(dist_theta(:, 2)*exp(theta(:, 2)))

    call discretize_AR(0.945d0**5d0, -0.150d0, sigma5(0.945d0, 0.0550d0), theta(:, 3), pi_theta(:, :, 3), dist_theta(:, 3))
                ...
                do ip = 0, NP
                  do ix = 0, NX
                    do ia = 0, NA
                      ...
                      xy(2) = max(k(1, ia, ix, ip, iw, ie, is, ij, it), 1d-8)
            ...
            do iw = 1, NW
              do ip = 0, NP
                do ia = 0, NA
                  ...
                  ! copy decisions
                  aplus(:, ia, :, ip, iw, ie, is, ij, it) = xy(1)
                  xplus(:, ia, :, ip, iw, ie, is, ij, it) = xplus_com
                  k(:, ia, :, ip, iw, ie, is, ij, it) = k_com
                  pplus(:, ia, :, ip, iw, ie, is, ij, it) = pplus_com
                  c(:, ia, :, ip, iw, ie, is, ij, it) = max(c_com, 1d-10)
                  cx(:, ia, :, ip, iw, ie, is, ij, it) = cx_com
                  l(:, ia, :, ip, iw, ie, is, ij, it) = l_com
                  mx(:, ia, :, ip, iw, ie, is, ij, it) = mx_com
                  oplus(:, ia, :, ip, iw, ie, is, ij, it) = oplus_com
                  pencon(:, ia, :, ip, iw, ie, is, ij, it) = pencon_com
                  inctax(:, ia, :, ip, iw, ie, is, ij, it) = inctax_com
                  captax(:, ia, :, ip, iw, ie, is, ij, it) = captax_com
                  VV(:, ia, :, ip, iw, ie, is, ij, it) = -fret
            ...
            do iw = 1, NW
              do ip = 0, NP
                do ia = 0, NA
                  ...
                  ! copy decisions
                  aplus(:, ia, :, ip, iw, ie, is, ij, it) = xy(1)
                  xplus(:, ia, :, ip, iw, ie, is, ij, it) = 0d0
                  k(:, ia, :, ip, iw, ie, is, ij, it) = k_com
                  pplus(:, ia, :, ip, iw, ie, is, ij, it) = pplus_com
                  c(:, ia, :, ip, iw, ie, is, ij, it) = max(c_com, 1d-10)
                  cx(:, ia, :, ip, iw, ie, is, ij, it) = cx_com
                  l(:, ia, :, ip, iw, ie, is, ij, it) = l_com
                  mx(:, ia, :, ip, iw, ie, is, ij, it) = 0d0
                  oplus(:, ia, :, ip, iw, ie, is, ij, it) = oplus_com
                  pencon(:, ia, :, ip, iw, ie, is, ij, it) = pencon_com
                  inctax(:, ia, :, ip, iw, ie, is, ij, it) = inctax_com
                  captax(:, ia, :, ip, iw, ie, is, ij, it) = captax_com
              ...
              do ie = 1, NE
                do is = 1, NS
                  do io = 0, NO
                    ...
                    if (VV_today >= 0d0)write(*,*)'VV_today', VV_today, io, ia, ix, ip, iw, ie, is, ij
                    if (VV_today <= -1d10)write(*,*)'VV_today', VV_today, io, ia, ix, ip, iw, ie, is, ij

                    ! get target utility
                    VV_target = VV(io, ia, ix, ip, iw, ie, is, ij, 0)
                    if (VV_target >= 0d0)write(*,*)'VV_target', VV_target, io, ia, ix, ip, iw, ie, is, ij
                    if (VV_target <= -1d10)write(*,*)'VV_target', VV_target, io, ia, ix, ip, iw, ie, is, ij

                    ! get derivative of the value function
                    dVV_da = margu(c(io, ia, ix, ip, iw, ie, is, ij, 1), l(io, ia, ix, ip, iw, ie, is, ij, 1), 1)
                     if (dVV_da < 0d0)write(*,*)'dVV_da', dVV_da, io, ia, ix, ip, iw, ie, is, ij
  ...
  subroutine output(it)
    ...
    enddo
    write(23,'(3x,4f8.2)')(/sum(m(1, :, :, :, :, :, 1, :, it))/sum(m(:, :, :, :, :, :, 1, :, it)), &
                            sum(m(1, :, :, :, :, :, 2, :, it))/sum(m(:, :, :, :, :, :, 2, :, it)), &
                            sum(m(1, :, :, :, :, :, 3, :, it))/sum(m(:, :, :, :, :, :, 3, :, it)), &
                            sum(m(1, :, :, :, :, :, :, :, it))/sum(m(:, :, :, :, :, :, :, :, it))/)*100d0
    write(23,'(a,4f8.2)')'<10', (/sum(m(1, :, :, :, :, :, 1, 2:JR-1, it))/sum(m(:, :, :, :, :, :, 1, 2:JR-1, it)), &
                                  sum(m(1, :, :, :, :, :, 2, 2:JR-1, it))/sum(m(:, :, :, :, :, :, 2, 2:JR-1, it)), &
                                  sum(m(1, :, :, :, :, :, 3, 2:JR-1, it))/sum(m(:, :, :, :, :, :, 3, 2:JR-1, it)), &
                                  sum(m(1, :, :, :, :, :, :, 2:JR-1, it))/sum(m(:, :, :, :, :, :, :, 2:JR-1, it))/)*100d0
    ...
    if (show_graphics) then
      ...
      if (it == 0 .OR. it == TT) then
        ...
        ! call plot((/(dble(ia), ia=0,NA)/), EV(1, :, 0, 0, 2, 5, 3, 5, it)**(1d0-gamma)/(1d0-gamma), marker=1)
        ! call plot((/(dble(ia), ia=0,NA)/), oplus(0, :, 0, 0, 2, 5, 3, 5, it), marker=2)
        ! call plot((/(dble(ia), ia=0,NA)/), oplus(1, :, 0, 0, 2, 5, 3, 5, it), marker=2)
        ! call execplot()
        !
        ! call plot((/(dble(ia), ia=0,NA)/), EV(0, :, 0, 0, 2, 4, 1, 5, it)**(1d0-gamma)/(1d0-gamma), marker=1)
        ! call plot((/(dble(ia), ia=0,NA)/), EV(1, :, 0, 0, 2, 4, 1, 5, it)**(1d0-gamma)/(1d0-gamma), marker=1)
