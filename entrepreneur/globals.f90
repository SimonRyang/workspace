...
contains
  ...
  function valuefunc_w(xy)
    ...
    inctax_com = tarif(max(wage*l_com - 0.10d0*wage*l_com - 0.04d0*inc_tax(0) - pencon_com, 0d0) + pen(ip_com, ij_com, it_com))

    ! calculate capital gains tax
    captax_com = taur(it_com)*1.055d0*max(r(it_com)*a(ia_com)-0.10d0*r(it_com)*a(ia_com)-2d0*0.0267d0*inc_tax(0), 0d0)
  ...
  function valuefunc_e(xy)
    ...
    ! calculate income tax
    inctax_com = tarif(max(profit - 0.10d0*profit - 0.04d0*inc_tax(0) - pencon_com, 0d0) + pen(ip_com, ij_com, it_com))

    ! calcualte capital gains tax
    captax_com = taur(it_com)*1.055d0*max(r(it_com)*max(a(ia_com)-k_com, 0d0) - 0.10d0*r(it_com)*max(a(ia_com)-k_com, 0d0) - 2d0*0.0267d0*inc_tax(0), 0d0)
